...
class DataSet:
    def __init__(self, input_columns: list[str], output_columns: list[str], input_data: list[list[float]] | None = None,
        ...
        self.__is_cnn = is_cnn
        if is_cnn:
            self.__input_data = np.array(self.__input_data)
            self.__output_data = np.array(self.__output_data)
    ...
    def normalise(self, training_data, validation_data):
        scaler = StandardScaler()
        if self.__is_cnn:
            normalised_train_data = training_data
            normalised_validate_data = validation_data
