...
namespace OnwardTravel.Service
{
    internal class UserService
    {
        ...
        {
            User user = _userRepository.TryLogin(username, PasswordEncryptor.Encrypt(password));
            return user ?? throw new InvalidDataException("Invalid username or password!");
        }
        ...
        public ClientSeatDTO[] FindReservationsByRide(Ride ride)
        {
            ClientSeatDTO[] occupiedSeats = new ClientSeatDTO[18];
            foreach (Reservation reservation in _reservationRepository.FindReservationsOnRide(ride))
            {
                foreach (ReservedSeat reservedSeat in reservation.Seats)
                {
                    occupiedSeats[reservedSeat.SeatNumber - 1] = new ClientSeatDTO(reservation.Client, reservation, reservedSeat);
                }
            }
            for (int i = 0; i < 18; ++i)
            {
                ...
                {
                    occupiedSeats[i] = new ClientSeatDTO(null, null, new ReservedSeat(i + 1));
                }
            }
        ...
        public void CreateReservation(Ride ride, string clientName, string phoneNumber, int seatsCount)
        {
            ...
            foreach (ClientSeatDTO clientSeat in FindReservationsByRide(ride))
            {
