...
vector<Medicine> PharmacyService::findMedicineList(string namePattern) {
	...
	const auto& medicineList = medicineListRepository.getMedicineList();
	const auto& iterator = copy_if(medicineList.begin(), medicineList.end(), searchedItems.begin(), [&](const Medicine& other) {
...
vector<Medicine> PharmacyService::filterMedicinePriceRange(double minPrice, double maxPrice) {
	...
	const auto& medicineList = medicineListRepository.getMedicineList();
	const auto& iterator = copy_if(medicineList.begin(), medicineList.end(), filteredItems.begin(), [&](const Medicine& other) noexcept {
			return other.getPrice() >= minPrice && other.getPrice() <= maxPrice; });
	filteredItems.resize(distance(filteredItems.begin(), iterator));
	return filteredItems;
}

vector<Medicine> PharmacyService::sortMedicineList(bool (*cmp)(const Medicine&, const Medicine&), bool reversed = false) {
	if (!cmp) {
...
unordered_map<string, vector<Medicine>> PharmacyService::producerReportMap() {
	unordered_map<string, vector<Medicine>> report;
	for (const auto& i : medicineListRepository.getMedicineList()) {
		report[i.getProducer()].push_back(i);
	}
	return report;
}
...
void PharmacyService::undoLastOperation() {
	...
	const auto& lastOperation = undoList.back().get();
	if (lastOperation) {
