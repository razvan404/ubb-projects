...
segment code use32 class=code
    start:
        mov esi, 0           
        mov edi, 0
        mov ecx, len1 + len2                                     ; pregatim parcurgerea celor doua siruri
        repeta:
            cmp esi, len1                                        ; verificam daca ne aflam in primul sir sau in al doilea
            JL task1
            JGE task2
                 
                task1:                                           ; in task 1 facem cerinta pentru primul sir
                    ...
                    JNE nedivizibil
                        ...
                        nedivizibil:                             ; daca nu este divizibil, doar vom incrementa pe esi
                            inc esi
                            JMP end_repeta
                task2:                                           ; in task 2 facem cerinta pentru al doilea sir
                    mov ebp, len2
                    sub ebp, 1
                    add ebp, len1
                    sub ebp, esi                                 ; ebp = len2 - (esi - len1)[pozitia din al doilea sir in care ne aflam] - 1  = len2 - 1 + len1 - esi
                 
                    mov bl, [s2 + ebp]                           ; in bl vom lua al 'ebp'-lea element din al doilea sir, adica al '(esi - len1)'-lea element in ordine inversa
                    mov [d + edi], bl
                    inc edi
