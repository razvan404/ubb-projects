...
class FiniteAutomaton:
    ...
    def accepts(self, string: str) -> AcceptResponse:
        ...
        for char in string:
            if char not in self.__alphabet:
                return self.AcceptResponse.REJECT_BY_ALPHABET
            if current_state not in self.__transitions or char not in self.__transitions[current_state]:
                return self.AcceptResponse.REJECT_BY_NO_TRANSITION
            current_state = self.__transitions[current_state][char]

        return self.AcceptResponse.ACCEPT_BY_FINAL_STATE if current_state in self.__accept_states else \
            self.AcceptResponse.REJECT_BY_FINAL_STATE
...
class AutomatonReader:
    ...
    def read_from_file(cls, file_name: str) -> FiniteAutomaton:
        with open(file_name, "r") as file:
            ...
            transitions, is_determinist = cls.__raw_transitions_to_dict(raw_transitions.strip().split(" "))
    ...
    def read_from_console(cls) -> FiniteAutomaton:
        ...
        raw_transitions = input("Transitions: ").strip().split(" ")
