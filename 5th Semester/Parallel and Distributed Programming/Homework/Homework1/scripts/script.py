...
if __name__ == "__main__":
  ...
  types = ['sequential', 'lines', 'columns', 'linearization']
  parallel_types = ['batch', 'cyclic']
  ...
  for test_case, thread_count in zip(test_cases, thread_counts):
    ...
    cmd = f'{just_script_with_file},{types[0]} {run_counts}'
    print(cmd)
    os.system(cmd)
    ...
    for i in range(1, len(types)):
      for parallel_type in parallel_types:
        for threads in thread_count:
          cmd = f'{just_script_with_file},{types[i]},{parallel_type},{threads} {run_counts}'
          print(cmd)
          os.system(cmd)